...
function createDisruptionMarkers(data) {
    const markerGeometry = new THREE.SphereGeometry(0.02, 8, 8);  // Small sphere for visible marker
    const invisibleMarkerGeometry = new THREE.SphereGeometry(0.05, 8, 8);  // Larger invisible marker for interaction 
    ...
    data.forEach((disruption) => {
        ...
        invisibleMarker.userData = { description: `${description}`, time: `${utcTime}` };
        console.log(invisibleMarker.tooltip)
...
function onMouseMove(event) {
    ...
    if (intersects.length > 0) {
        ...
        const utcTime = new Date(object.userData.time);  // Convert string to Date object
        ...
        if (object.visible && object.userData.description) {
            tooltip.innerHTML =  `
            <div style="font-weight: bold; font-size: 30px; border-bottom: 1px solid white; padding-bottom: 2px; margin-bottom: 2px;">
                ${formattedTime}  <!-- Title: UTC time -->
            </div>
            <div style="font-size: 20px;">
                ${object.userData.description}  <!-- Body: Description -->
            </div>
        `;
            ...
            tooltip.style.maxWidth = "17%"
            tooltip.style.borderRadius = "10%"
        } else {
